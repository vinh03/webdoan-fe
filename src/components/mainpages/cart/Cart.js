...
function Cart() {
 ...
 // tính tổng tiền
  useEffect(() => {
    const getTotal = () => {
      ...
      setTotal(total);// Cập nhật giá trị tổng vào trạng thái 'total'.
    };
    getTotal();// cập nhật giá khi thay đổi
  }, [cart]);
 // tăng số lượng
  const increment = (id) => {
 ...
 // giảm số lượng
  const decrement = (id) => {
    // Giảm số lượng của sản phẩm có 'id' chỉ định trong giỏ hàng.
    cart.forEach((item) => {
      if (item._id === id) {
        item.quantity === 1 ? (item.quantity = 1) : (item.quantity -= 1);
      }
    });
   // Cập nhật trạng thái 'cart' với giỏ hàng đã cập nhật.
    setCart([...cart]);
    // Gọi hàm 'addToCart' với giỏ hàng hiện tại.
    addToCart(cart);
  };
